...
err:
	...
	bool CRichEditUI::DoPaint(HDC hDC, const RECT& rcPaint, CControlUI* pStopControl)
	{
		...
		if (m_items.GetSize() > 0 && canPaintChirden) {
			RECT rc = m_rcItem;
			rc.left += m_rcInset.left;
			rc.top += m_rcInset.top;
			rc.right -= m_rcInset.right;
	...
	void CRichEditUI::SetCanPaintChirden(bool paint){
		canPaintChirden = paint;
	}

	bool CRichEditUI::GetCanPaintChirden(bool paint){
		return canPaintChirden;
	}

	HDC CRichEditUI::GetTxtHDC(){
		return m_pTwh->TxGetDC();
	}
