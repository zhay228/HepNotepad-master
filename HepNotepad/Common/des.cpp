...
char * StrToHex(const char *lpInData, int iInSize)
{
	...
	sTemp = (char *)malloc((iInSize+1) * 2);
	memset(sTemp, 0, (iInSize+1) * 2);
	if(!sTemp)
		return NULL;
	
	for(i = 0; i < iInSize; i++)
	{
		sprintf( &(sTemp[2*i]), "%02X",(unsigned char)lpInData[i] );
		sprintf( &(sTemp[2*i+1]), "%02X",(unsigned char)(lpInData[i] << 4 ) );       
	}
	return sTemp;
}
int HexToInt(const char *lpInData)
{
	...
	for(i=0; i<sLen; i++) 
	{
		if(lpInData[i]<='9' && lpInData[i]>='0')
			stat = lpInData[i] - '0';
		else if(lpInData[i]>='a' && lpInData[i]<='f')
			stat = lpInData[i] - 87;
		else if(lpInData[i]>='A' && lpInData[i]<='F')
			stat = lpInData[i] - 55;
		else
...
char * HexToStr(const char *lpInData, int iInSize)
{
	...
	char *lpRet;
	int i=0;
	lpRet = (char *)malloc((rLen+1) * sizeof(char));
	if(!lpRet)
		return NULL;
	memset(lpRet, 0, rLen+1);
	
	while(i < rLen)
	{
		sprintf(Temp, "%c%c", lpInData[i*2], lpInData[i*2+1]);
		sprintf(&(lpRet[i]), "%c", HexToInt(Temp));
...
char*  Encrypt(string content){
	...
	unsigned char *decodeStr = (unsigned char*)malloc(len + 8);
	memset(decodeStr, 0, len + 8);
	int i = ENCRYPT((unsigned char*)KEY, (unsigned char*)content.c_str(), decodeStr, len);

	char* hexStr = StrToHex((const char *)decodeStr, i);
...
char*  Decrypt(string content){
	...
	int len = content.length();
	char*  tempEnCodeStr = HexToStr(content.c_str(), len);
	char data[3072];
	if (tempEnCodeStr != NULL)
	{
		decodeStr = (unsigned char *)malloc(len);
		memset(decodeStr, 0, len);
		int i = DECRYPT((unsigned char *)KEY, (unsigned char *)decodeStr, (unsigned char *)(tempEnCodeStr), len / 2);
		if (decodeStr)
		{
			sprintf(data, "%s", decodeStr); 
		}
	}
	return data;
}