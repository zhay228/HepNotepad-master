...
void CFileDialogEx::SetFilter(LPCTSTR lpszFilter)
{
	...
	//MessageBox(NULL, m_szFilter, "", MB_OK);
...
tstring CFileDialogEx::GetPathName()
{
	return m_stOFN.lpstrFile;
}

tstring CFileDialogEx::GetFileName()
{
	return _T("");
}

tstring CFileDialogEx::GetFileExt()
{
	return _T("");
}

tstring CFileDialogEx::GetFileTitle()
{
	return _T("");
}

tstring CFileDialogEx::GetFolderPath()
{
	return _T("");
}
...
tstring CFileDialogEx::GetNextPathName(POSITION& pos)
{
	...
	tstring strBasePath = m_stOFN.lpstrFile;
	tstring strFileName = lpsz;
	if (strBasePath.substr(strBasePath.length() - 1) != _T("\\"))
		strBasePath += _T("\\");
	tstring strPath = strBasePath + strFileName;
